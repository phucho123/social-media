        ...
        <div className='bg-white pt-2 h-full shadow-md overflow-y-auto'>
            <p className='font-semibold text-2xl px-2 pb-4'>Chat</p>
            {
                user.user?.friends.length && user.user.friends.map((friend, index) => (
                    <Link className={`flex w-full items-center gap-2 mb-2 justify-start px-2 py-2 hover:bg-gray-300 hover:bg-opacity-50 cursor-pointer ${id == friend._id ? 'bg-gray-300 bg-opacity-50' : ""}`} key={index} to={`/chat/${friend._id}`}>
